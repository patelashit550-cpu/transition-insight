...
The repo can diagnose. These three close the perimeter and cannot be done from git:

1. **GitHub → Settings → Pages → Build and deployment → Source: GitHub Actions.** Production is still the legacy `gh-pages` branch, which is why live `security.txt` still names `transition-insight.com`. After you switch, re-run **Deploy to GitHub Pages**.
2. **SNS (`transition-insight.sol`):** set the URL (or IPFS) record to `https://ashitmilne.xyz/`. Until then `transition-insight.sol.site` is a public Bonfida profile — a different origin.
3. **GitHub → Settings → Environments → `github-pages`:** deployment branches = `main` only (remove `gh-pages` after the Actions cutover).
...
4. **Repo → Settings → Pages:** build from **GitHub Actions**, not the `gh-pages` branch (disable the legacy branch publisher).
5. **Repo → Settings → Environments → `github-pages`:** deployment branches = `main` only. Optional: required reviewer (you) so a stolen PAT cannot ship instantly.
