...
7. **Secrets:** do **not** put `SOLANA_SIGNING_KEY`, `PINATA_JWT`, `JUPITER_API_KEY`, OFT program keypairs, or LayerZero deployer keys in GitHub Actions. Sign and pin on the laptop (`.env.local`, gitignored). Jupiter `x-api-key` is node/CLI only — never `NEXT_PUBLIC_*`.
