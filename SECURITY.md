...
7. **Secrets:** do **not** put `SOLANA_SIGNING_KEY`, `SOLANA_KEYPAIR_PATH`, `PINATA_JWT`, OFT program keypairs, or LayerZero deployer keys in GitHub Actions. Sign, pin, and assemble Solana transactions on the laptop (`.env.local`, gitignored). `@solana/web3.js` v1 and `@solana/spl-token` are for local tx construction — they must not ship a private key into the static Pages build.
