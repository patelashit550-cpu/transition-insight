...
7. **Secrets:** do **not** put `SOLANA_SIGNING_KEY`, `SOLANA_KEYPAIR_PATH`, `PINATA_JWT`, `JUPITER_API_KEY`, `ETHERSCAN_API_KEY`, `COLOSSEUM_COPILOT_PAT`, `VYBE_API_KEY`, `OPENAI_API_KEY`, `BRAVE_SEARCH_API_KEY`, OFT program keypairs, or LayerZero deployer keys in GitHub Actions. Sign, pin, assemble Solana transactions, and call Jupiter / Etherscan / Colosseum Copilot / Vybe / Argonaut on the laptop (`.env.local`, gitignored). `@solana/web3.js` v1, `@solana/spl-token`, Jupiter Tokens, and Etherscan `tokentx` are CLI-only — they must not ship a private key or API key into the static build. Never `NEXT_PUBLIC_*` for those keys. Argonaut (`/api/argonaut/ask`) is a local `next dev` route, stashed during static export like Chord.
